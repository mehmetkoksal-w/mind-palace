...
4.  **Review**: Use `palace explore --type decision` to see the timeline.
...
```sh
palace explore --rooms           # Browse logical modules
palace explore --map <symbol>    # Visualize call graph
palace brief <file>              # Get file intelligence
```

- **Hotspot Analysis**: Use `palace brief --failures` to see frequently changing files.
- **Dependency Graph**: Use `palace explore --map` to explore how functions interact.
- **Knowledge Timeline**: Use `palace explore --type decision` to see project decisions.
