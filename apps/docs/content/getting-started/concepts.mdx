...
Created by `palace context "goal"`. Schema-validated.

### Change Signal

`change-signal.json` - Diff metadata for scoped workflows.

Created by `palace ci signal --diff HEAD~1..HEAD`. Enables CI to verify only changed files.
...
- `get_context` - Generate context packs
...
VS Code extension. Automates check/scan on file save.
