...
```sh
palace recall update d_abc123 successful
palace recall update d_xyz789 failed --notes "Performance was too low"
```

### Linking Records

Knowledge exists in a web of relationships:

```sh
palace recall link d_new --supersedes d_old
palace recall link l_pattern --supports d_choice
```
...
By committing these curated knowledge items to Git (stored in `.palace/memory/`), you build a permanent, shared intelligence for your codebase.
