...
| Tool           | Parameters                    | Returns                                           |
| -------------- | ----------------------------- | ------------------------------------------------- |
| `session_init` | `agent_name`, `task?`         | Initialize session with full context (RECOMMENDED)|
...
| `session_end`      | `sessionId`                                           | End session with summary    |
| `session_conflict` | `path`                                                | Check for file conflicts    |
| `session_list`     | `active?`, `limit?`                                   | List sessions               |
| `session_resume`   | `sessionId?`, `agentType?`                            | Resume a previous session   |
...
| Tool            | Parameters              | Returns                           |
| --------------- | ----------------------- | --------------------------------- |
| `context_focus` | `task`, `pin?`          | Set task focus for prioritization |
| `context_get`   | `file?`, `maxTokens?`   | Get prioritized context           |
| `context_pin`   | `id`, `unpin?`          | Pin/unpin records to context      |

#### Handoff Tools

| Tool               | Parameters                                                       | Returns                    |
| ------------------ | ---------------------------------------------------------------- | -------------------------- |
| `handoff_create`   | `task`, `to?`, `context?`, `pending?`, `pin?`, `priority?`       | Create task handoff        |
| `handoff_list`     | `status?`                                                        | List available handoffs    |
| `handoff_accept`   | `id`                                                             | Accept and get full context|
| `handoff_complete` | `id`, `summary?`                                                 | Mark handoff completed     |

#### Analytics Tools

| Tool                  | Parameters        | Returns                         |
| --------------------- | ----------------- | ------------------------------- |
| `analytics_sessions`  | `days?`           | Session statistics and trends   |
| `analytics_learnings` | `limit?`, `sort?` | Learning effectiveness tracking |
| `analytics_health`    | -                 | Workspace health statistics     |
...
palace check --fast
# If stale:
palace scan && palace explore "next goal" --full
```
...
palace explore "fix bug" --full --diff HEAD~1..HEAD
```
...
Track agent work with sessions. Use `session_init` for single-call initialization:
...
1. **Use session_init** - Always begin with `session_init` for single-call initialization with full context
2. **Check pending handoffs** - Review handoffs shown in session_init before starting new work
3. **Log activities** - Record reads, edits, and commands for file intelligence
4. **Capture learnings** - Store patterns and heuristics discovered during work
5. **Check for conflicts** - Before editing, verify no other agent is working on the file
6. **Use context_focus** - Set task focus for better context prioritization
7. **Create handoffs** - When you can't complete work, hand it off to another agent
8. **End sessions properly** - Call `session_end` when done (generates summary)
9. **Check workspace health** - Use `analytics_health` periodically to monitor workspace quality
10. **Respect guardrails** - Never touch protected files
11. **Honor scope** - Stay within the requested scope (diff or full)
