...
```
Session Init → File Context → Work → Log/Store → End
```

1. **session_init**: Start every task with full session + workspace context
2. **file_context**: Get file context (including conflict checks) before editing
3. **store**: Save learnings, decisions, and ideas as you work
4. **session** with `action: "log"`: Record major activities for audit trail
5. **session** with `action: "end"`: End session when task is complete

### Priority System

- **CRITICAL**: Must do (session_init, file_context, session with action=end)
- **IMPORTANT**: Should do (store, session with action=log)
- **RECOMMENDED**: Optional but valuable (recall, explore, postmortems)
- **ADMIN**: Human-mode only (govern approve/reject, store with direct=true)
...
- **CRITICAL**: session_init, file_context, session(action=end), store (for our team, always store learnings)
- **IMPORTANT**: session(action=log), recall, session(action=conflict)
- **RECOMMENDED**: explore, postmortem, search
```
