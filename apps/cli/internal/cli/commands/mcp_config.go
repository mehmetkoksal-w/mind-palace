...
func findPalaceBinary() (string, error) {
	...
	return "", fmt.Errorf("palace binary not found. Install with: go install github.com/mehmetkoksal-w/mind-palace/apps/cli@latest")
}
...
func addClaudeGlobalRules(config *map[string]interface{}) {
	globalRules := []string{
		...
		"1. ALWAYS start with: session_init({agent_name: 'claude-code', task: 'description'})",
		"2. MUST call file_context({file_path: 'path', session_id: '...'}) before editing ANY file",
		"3. MUST end with: session({action: 'end', sessionId: '...', outcome: 'completed|failed', summary: '...'})",
		"4. Use session({action: 'log', ...}) for major activity logging",
		"",
		"## Required Before Editing:",
		"- file_context() provides file-specific learnings, decisions, and failure warnings",
		"- file_context() includes conflict checks for concurrent edits",
		...
		"- After failures: postmortem({action: 'create', title: '...', what_happened: '...', root_cause: '...'})",
		"",
		"## Exploration:",
		"- explore({action: 'search', query: 'description'}) - find relevant code",
		"- recall({action: 'get', type: 'learnings', query: 'topic'}) - search learnings",
		"- recall({action: 'get', type: 'decisions', query: 'topic'}) - search decisions",
		"",
		"Priority Indicators:",
		"🔴 CRITICAL: Must do (session_init, file_context, session action=end)",
		"🟡 IMPORTANT: Should do (store, session action=log)",
