...
func TestCallGraph(t *testing.T) {
	...
	db.ExecContext(context.Background(), `INSERT INTO files(path, hash, size, mod_time, indexed_at, language) VALUES (?, ?, ?, ?, ?, ?);`, "main.go", "h1", 100, now, now, "go")
	db.ExecContext(context.Background(), `INSERT INTO symbols(id, file_path, name, kind, line_start, line_end, signature, doc_comment, parent_id, exported) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`, 1, "main.go", "main", "function", 1, 10, "()", "", nil, 1)
	db.ExecContext(context.Background(), `INSERT INTO symbols(id, file_path, name, kind, line_start, line_end, signature, doc_comment, parent_id, exported) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`, 2, "main.go", "helper", "function", 11, 20, "()", "", nil, 1)

	// 'call' from helper (line 15) to lib.Func
	db.ExecContext(context.Background(), `INSERT INTO relationships(source_file, source_symbol_id, target_symbol, kind, line, column) VALUES (?, ?, ?, ?, ?, ?);`, "main.go", 2, "lib.Func", "call", 15, 1)
	// 'call' from main (line 5) to helper
	db.ExecContext(context.Background(), `INSERT INTO relationships(source_file, source_symbol_id, target_symbol, kind, line, column) VALUES (?, ?, ?, ?, ?, ?);`, "main.go", 1, "helper", "call", 5, 1)
