...
import { ApiService, FileIntel, Learning } from '../../core/services/api.service';
...
@Component({
    ...
    <div class="intel-container">
      ...
      <div class="view-toggle">
        <button [class.active]="viewMode() === 'heatmap'" (click)="viewMode.set('heatmap')">
          Heat Map
        </button>
          ...
          <div class="heatmap-container">
            ...
            @if (hotspots().length === 0) {
              <div class="empty-state">
                ...
                <p class="hint">File intelligence is populated as agents work on your codebase.</p>
              </div>
            }
          </div>
        }
                ...
                @for (file of hotspots(); track file.path) {
                  <div class="file-card" (click)="selectFile(file)">
                    <div class="file-header">
                      <div class="heat-indicator" [style.background-color]="getHeatColor(file)"></div>
                    ...
                    <div class="edit-bar">
                      <div class="edit-fill" [style.width.%]="getEditPercentage(file)"></div>
                      @if (file.failureCount > 0) {
                        <div class="failure-fill" [style.width.%]="getFailurePercentage(file)"></div>
                      }
                    </div>
                  </div>
                }
                ...
                @for (file of fragile(); track file.path) {
                  <div class="file-card fragile" (click)="selectFile(file)">
                    ...
                    <div class="file-stats">
                      <span class="failures">{{ file.failureCount }} failures</span>
                      <span class="edits">{{ file.editCount }} edits</span>
                      <span class="rate">{{ getFailureRate(file) }}% failure rate</span>
                    </div>
                  </div>
                }
                @if (fragile().length === 0) {
            ...
            <div class="tree-item" [style.padding-left.px]="depth * 20">
              ...
              @if (node.file) {
                <div class="tree-file" (click)="selectFile(node.file)">
                  <div class="heat-indicator" [style.background-color]="getHeatColor(node.file)"></div>
                  <span class="node-name">{{ node.name }}</span>
                  <span class="node-stats">
                    {{ node.file.editCount }} edits
                    @if (node.file.failureCount > 0) {
                      <span class="failure-count">{{ node.file.failureCount }} failures</span>
                    }
                  </span>
                </div>
              } @else {
        ...
        @if (selectedFile()) {
          <div class="file-detail-panel">
            <div class="detail-header">
              ...
              <button class="close-btn" (click)="selectedFile.set(null)">Close</button>
            </div>
            <div class="detail-stats">
              ...
              <div class="detail-stat" [class.danger]="selectedFile()!.failureCount > 0">
            ...
            @if (selectedFile()!.lastEditor) {
              <div class="last-edit-info">
                Last edited by <strong>{{ selectedFile()!.lastEditor }}</strong>
                @if (selectedFile()!.lastEdited) {
                  on {{ selectedFile()!.lastEdited | date:'medium' }}
                }
              </div>
            }
              ...
              <div class="learnings-section">
                ...
                <ul class="learnings-list">
                  @for (learning of fileLearnings(); track learning.id) {
                    <li>
                      <span class="confidence" [style.opacity]="learning.confidence">●</span>
                      {{ learning.content }}
                    </li>
                  }
                </ul>
...
export class IntelComponent implements OnInit {
  ...
  loadData() {
    ...
    this.api.getHotspots().subscribe({
      ...
      error: (err) => {
        console.error('Failed to load file intel:', err);
        this.loading.set(false);
      }
    });
  }
