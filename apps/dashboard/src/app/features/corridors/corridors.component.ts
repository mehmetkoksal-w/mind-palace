    ...
    <div class="corridors">
      ...
      <div class="section">
        ...
        <div class="stats-row">
          ...
          <div class="stat">
            <span class="value">{{ formatConfidence(stats()?.averageConfidence) }}</span>
        ...
        <div class="learnings-preview">
          @for (learning of personalLearnings(); track learning.id) {
            <div class="learning-item">
              <div class="confidence-badge">{{ (learning.confidence * 100).toFixed(0) }}%</div>
          ...
          @for (link of links(); track link.name) {
            <div class="link-card">
              ...
              <div class="link-meta">
                ...
                Last accessed: {{ link.lastAccessed ? formatDate(link.lastAccessed) : 'Never' }}
              </div>
            </div>
          }
          @if (links().length === 0) {
            <div class="empty">
              ...
              Use <code>palace corridor link &lt;name&gt; &lt;path&gt;</code> to link one.
            </div>
          }
        </div>
      </div>
...
export class CorridorsComponent implements OnInit {
  ...
  loadData() {
    this.api.getCorridors().subscribe({
      ...
      error: (err) => console.error('Failed to load corridors:', err)
    });

    this.api.getPersonalLearnings('', 10).subscribe({
      next: (data) => this.personalLearnings.set(data.learnings || []),
      error: (err) => console.error('Failed to load personal learnings:', err)
    });
  }
