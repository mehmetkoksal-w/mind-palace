    ...
    <div class="neural-map-container" #container [style.height.px]="height()">
      ...
      >
        <div class="tooltip-header">
          <span class="type-badge" [class]="hoveredNode()!.type">{{
            hoveredNode()!.type
          }}</span>
        </div>
      ...
      <div class="detail-panel">
        <div class="panel-header">
          <span class="type-badge" [class]="selectedNode()!.type">{{
            selectedNode()!.type
          }}</span>
        ...
        <div class="panel-content">
          ...
          <p class="detail-text">
            {{ selectedNode()!.data.description || "Code module" }}
          </p>
          <div class="stats-row">
            <span>{{ selectedNode()!.data.files?.length || 0 }} files</span>
            <span
              >{{ selectedNode()!.data.entryPoints?.length || 0 }} entry
              points</span
            >
          </div>
          } @else {
...
export class NeuralMapComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    this.neuralMapService.fetchMapData().subscribe({
      ...
      error: (err) => {
        inject(LoggerService).error(
          "Failed to load neural map data",
          err,
          "NeuralMapComponent"
        );
        this.loading.set(false);
  ...
  private renderGraph() {
    ...
    this.simulation = d3
      .forceSimulation<NeuralNode>(this.nodes)
      .force(
        "link",
        d3
    ...
    const link = this.g
      ...
      .attr("stroke-width", (d) =>
        d.type === "contains" ? 1 : d.type === "depends" ? 2.5 : 1.5
      )
      .attr("stroke-dasharray", (d) =>
        d.type === "contradicts" ? "4,4" : d.type === "refines" ? "2,2" : "none"
      )
      .attr("stroke-opacity", (d) => (d.type === "depends" ? 0.8 : 0.6))
      .attr("marker-end", (d) =>
        ["references", "supports", "implements", "refines", "depends"].includes(
          d.type
        )
          ? `url(#arrow-${d.type})`
          : null
    ...
    // Draw nodes
    const node = this.g
      .selectAll<SVGGElement, NeuralNode>(".node")
      .data(this.nodes)
      .join("g")
      .attr("class", "node")
    ...
    node
      .filter((d) => d.type === "room" || d.importance > 0.5)
      .append("text")
      .text((d) =>
  ...
  private dragStarted(
    event: d3.D3DragEvent<SVGGElement, NeuralNode, NeuralNode>,
    d: NeuralNode
  ) {
  ...
  private selectNode(node: NeuralNode) {
    ...
    this.g?.selectAll(".link").attr("opacity", (d: any) => {
  ...
  zoomIn() {
    ...
    this.svg
      .transition()
      .duration(300)
      .call(this.zoom.scaleBy as any, 1.4);
  }
