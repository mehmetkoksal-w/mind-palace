    ...
    <div class="overview">
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="stat-sub">{{ stats()?.sessions?.active || 0 }} active</div>
        </div>
          ...
          <div class="agents-list">
            @for (agent of agents(); track agent.agentId) {
              <div class="agent-card">
                ...
                <div class="agent-file">{{ agent.currentFile || 'No active file' }}</div>
                <div class="agent-heartbeat">Last seen: {{ formatTime(agent.heartbeat) }}</div>
              </div>
            }
          </div>
        </div>
        ...
        <div class="section">
          ...
          <div class="corridor-stats">
            <div class="stat-item">
              ...
              <span class="value">{{ stats()?.corridor?.learningCount || 0 }}</span>
            </div>
            <div class="stat-item">
              <span class="label">Linked Workspaces:</span>
              <span class="value">{{ stats()?.corridor?.linkedWorkspaces || 0 }}</span>
            </div>
          </div>
        </div>
      }
