...
@Component({
    ...
    <div class="contradictions-container">
      ...
      @if (loading()) {
        ...
        <div class="stats-row">
          <div class="stat-card">
            <div class="stat-value warning">{{ summary()?.totalContradictionLinks || 0 }}</div>
            <div class="stat-label">Active Contradictions</div>
          </div>
          <div class="stat-card">
            <div class="stat-value">{{ summary()?.recordsWithContradictions || 0 }}</div>
        ...
        @if (!summary()?.topContradictions?.length) {
          ...
          <div class="contradictions-list">
            @for (contradiction of summary()?.topContradictions; track contradiction.id) {
              <div class="contradiction-card" [class.expanded]="expandedId() === contradiction.id">
                <div class="card-header" (click)="toggleExpand(contradiction.id)">
                  ...
                  <div class="conflict-summary">
                    <div class="types">
                      <span class="type-badge" [class]="contradiction.sourceKind">{{ contradiction.sourceKind }}</span>
                      <svg class="conflict-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M8 12h8m-4-4l4 4-4 4"/>
                      </svg>
                      <span class="type-badge" [class]="contradiction.targetKind">{{ contradiction.targetKind }}</span>
                    </div>
                    <div class="confidence">
                      Confidence: {{ (contradiction.confidence * 100).toFixed(0) }}%
                    </div>
                  </div>
                  <div class="expand-icon" [class.expanded]="expandedId() === contradiction.id">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                @if (expandedId() === contradiction.id) {
                  <div class="card-content">
                    <div class="explanation">
                      ...
                      <p>{{ contradiction.explanation || 'These records contain contradictory information.' }}</p>
                    </div>
                    ...
                    <div class="records-comparison">
                      <div class="record-side source">
                        ...
                        @if (recordDetails()[contradiction.sourceId]) {
                          ...
                          <button class="load-btn" (click)="loadRecord(contradiction.sourceId)">Load content</button>
                        }
                      </div>
                      ...
                      <div class="record-side target">
                        ...
                        @if (recordDetails()[contradiction.targetId]) {
                          ...
                          <button class="load-btn" (click)="loadRecord(contradiction.targetId)">Load content</button>
                        }
                      </div>
                    </div>

                    <div class="actions">
                      <button class="action-btn" (click)="resolveContradiction(contradiction, 'source')">
                        Keep {{ contradiction.sourceKind }}
                      </button>
                      <button class="action-btn" (click)="resolveContradiction(contradiction, 'target')">
                        Keep {{ contradiction.targetKind }}
                      </button>
                      <button class="action-btn secondary" (click)="markResolved(contradiction)">
                        Mark as Resolved
                      </button>
                    </div>
...
export class ContradictionsComponent implements OnInit {
  ...
  loadContradictions() {
    ...
    this.http.get<ContradictionSummary>('/api/contradictions?limit=50').subscribe({
      next: (data) => {
      ...
      next: (data) => {
        ...
        if (record) {
          ...
          this.recordDetails.set({
            ...
            [id]: {
              id: record.id,
              kind: id.startsWith('i_') ? 'idea' : id.startsWith('d_') ? 'decision' : 'learning',
  ...
  resolveContradiction(contradiction: ContradictionLink, keep: 'source' | 'target') {
    // Archive the other record
    const archiveId = keep === 'source' ? contradiction.targetId : contradiction.sourceId;
    console.log(`Would archive ${archiveId} and remove contradiction link`);
  ...
  markResolved(contradiction: ContradictionLink) {
    console.log(`Would mark contradiction ${contradiction.id} as resolved`);
    // In a real implementation, this would call an API to remove the contradiction link
    alert(`This would mark the contradiction as resolved without archiving either record.`);
  }
