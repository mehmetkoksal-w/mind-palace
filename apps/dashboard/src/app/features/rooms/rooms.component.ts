  ...
  template: `
    <div class="rooms-container">
      ...
      <div class="rooms-grid">
        ...
        <div
          class="room-card"
          [class.expanded]="expandedRoom() === room.name"
        ...
        >
          <div class="room-header">
            ...
            <div class="room-badges">
              <span class="badge files"
                >{{ room.files.length || 0 }} files</span
              >
              @if (room.entryPoints.length) {
              <span class="badge entries"
                >{{ room.entryPoints.length }} entry</span
              >
              }
            </div>
          </div>
          ...
          <div class="room-details">
            ...
            <div class="section">
              ...
              <ul class="file-list">
                @for (file of room.files.slice(0, showAllFiles() ? undefined :
                10); track file) {
                <li class="file-item">
                  <svg
                    width="16"
              ...
              <button class="show-more" (click)="toggleShowAll($event)">
                {{
                  showAllFiles()
                    ? "Show less"
                    : "Show all " + room.files.length + " files"
                }}
              </button>
              }
            </div>
        ...
        <div class="no-results">
          @if (rooms().length === 0) {
          <p>
            No rooms found. Run <code>palace scan</code> to index your codebase.
          </p>
          } @else {
...
export class RoomsComponent implements OnInit {
  ...
  totalFiles(): number {
    return this.rooms().reduce(
      (sum, room) => sum + (room.files?.length || 0),
      0
    );
  }
  ...
  totalEntryPoints(): number {
    return this.rooms().reduce(
      (sum, room) => sum + (room.entryPoints?.length || 0),
      0
    );
  }
}
