        ...
        <div class="search-modal" (click)="$event.stopPropagation()">
          ...
          <div class="search-results" (keydown)="onResultsKeydown($event)">
            ...
            <div class="result-section">
              ...
              @for (symbol of results().symbols.slice(0, 5); track symbol.name +
              symbol.file; let i = $index) {
              <div
                class="result-item"
              ...
              >
                ...
                <div class="result-content">
                  ...
                  <span class="result-meta"
                    >{{ getFileName(symbol.file) }}:{{ symbol.line }}</span
                  >
                </div>
            ...
            <div class="result-section">
              ...
              @for (learning of results().learnings.slice(0, 5); track
              learning.id; let i = $index) {
              <div
                class="result-item"
                [class.selected]="
              ...
              >
                ...
                <div class="result-content">
                  <span class="result-title">{{
                    truncate(learning.content, 60)
                  }}</span>
                  <span class="result-meta">{{
                    learning.scope || "Global"
                  }}</span>
                </div>
                <span class="result-badge confidence"
            ...
            <div class="result-section">
              <div class="result-section-header">
                ...
                Personal Knowledge
                <span class="result-count">{{
                  results().corridor.length
                }}</span>
              </div>
              @for (item of results().corridor.slice(0, 5); track item.id; let i
              = $index) {
              <div
                class="result-item"
                [class.selected]="
...
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // First-run detection and onboarding redirect
    this.checkFirstRun();
  ...
  async checkFirstRun() {
    try {
      const isFirstRun = await this.onboarding.checkFirstRun();
      const completed = localStorage.getItem("onboarding_complete");
      if (isFirstRun && !completed) {
        this.router.navigate(["/onboarding"]);
      }
    } catch {
      // On error, stay on current route and avoid blocking app
    }
  }
  ...
  navigateToFirstResult() {
    ...
    if (idx < symbolsCount && r.symbols?.[idx]) {
      ...
      idx < symbolsCount + learningsCount &&
      r.learnings?.[idx - symbolsCount]
    ) {
  ...
  navigateToSymbol(symbol: { name: string; file: string; line: number }) {
    this.closeSearch();
    this.router.navigate(["/explore/graph"], {
      queryParams: { symbol: symbol.name },
    });
  }

  navigateToLearning(learning: { id: string }) {
    this.closeSearch();
    this.router.navigate(["/insights/learnings"], {
      queryParams: { id: learning.id },
    });
  }
