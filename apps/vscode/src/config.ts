...
export function getConfig(): MergedConfig {
    ...
    return {
        ...
        autoSync: projectConfig?.autoSync ?? vsCodeConfig.get<boolean>('autoSync') ?? DEFAULTS.autoSync,
        autoSyncDelay: projectConfig?.autoSyncDelay ?? vsCodeConfig.get<number>('autoSyncDelay') ?? DEFAULTS.autoSyncDelay,
        waitForCleanWorkspace: projectConfig?.waitForCleanWorkspace ?? vsCodeConfig.get<boolean>('waitForCleanWorkspace') ?? DEFAULTS.waitForCleanWorkspace,

        decorations: {
            enabled: projectConfig?.decorations?.enabled ?? DEFAULTS.decorations.enabled,
            style: projectConfig?.decorations?.style ?? DEFAULTS.decorations.style,
        },

        statusBar: {
            position: projectConfig?.statusBar?.position ?? DEFAULTS.statusBar.position,
            priority: projectConfig?.statusBar?.priority ?? DEFAULTS.statusBar.priority,
        },

        sidebar: {
            defaultView: projectConfig?.sidebar?.defaultView ?? DEFAULTS.sidebar.defaultView,
            graphLayout: projectConfig?.sidebar?.graphLayout ?? DEFAULTS.sidebar.graphLayout,
        },
    };
}
...
function readProjectConfig(): PalaceVSCodeConfig | null {
    ...
    const configPath = path.join(workspaceFolder.uri.fsPath, '.palace', 'palace.jsonc');
 ...
 */
export function watchProjectConfig(onConfigChange: () => void): vscode.Disposable {
