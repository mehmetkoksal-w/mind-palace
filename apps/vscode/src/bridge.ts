...
export const MCP_TOOLS = {
  ...
  // Postmortem tools
  POSTMORTEM_ADD: "postmortem_add",
  POSTMORTEM_GET: "get_postmortem",
  POSTMORTEM_RESOLVE: "resolve_postmortem",
  POSTMORTEM_TO_LEARNINGS: "postmortem_to_learnings",
} as const;
...
class MCPClient {
  ...
  private requestId = 0;
  private pendingRequests = createCache<{
  ...
  // Wrapper to allow stubbing in tests without touching core modules
  protected spawn(
    bin: string,
    args: string[],
  ...
  async connect(): Promise<void> {
    ...
    // Use current process cwd when no workspace is open (helps tests)
    const cwd =
    ...
    try {
      this.process = this.spawn(bin, ["serve"], {
        cwd,
  ...
  private handleResponse(response: JsonRpcResponse): void {
    ...
    if (!pending) {
      this.channel.appendLine(
        `[MCP] Received response for unknown request ID: ${response.id}`
      );
      return;
    }
  ...
  private handleDisconnect(): void {
    ...
    this.pendingRequests.forEach((id, pending) => {
      clearTimeout(pending.timeout);
    ...
    if (this.restartAttempts < this.maxRestartAttempts) {
      ...
      this.channel.appendLine(
        `[MCP] Attempting restart (${this.restartAttempts}/${this.maxRestartAttempts})...`
      );
      setTimeout(() => {
...
export class PalaceBridge {
  ...
  async search(
    query: string,
    options?: { limit?: number; room?: string; fuzzy?: boolean }
  ...
  async runHeal(): Promise<void> {
    ...
    try {
      this.channel.appendLine(
        `Running ${bin} scan && ${bin} check --collect...`
      );
      const { stdout, stderr } = await exec(
        `${bin} scan && ${bin} check --collect`,
        {
          cwd: vscode.workspace.workspaceFolders?.[0]?.uri.fsPath,
        }
      );
  ...
  async recordOutcome(
    ...
    await this.callTool(MCP_TOOLS.RECALL_OUTCOME, {
      decisionId,
      outcome,
      note,
    });
  ...
  /**
   * Add a postmortem capturing lessons learned from a task/bug/incident
   */
  async addPostmortem(
    title: string,
    notes: string,
    ...
    return this.callTool<{ id: string }>(MCP_TOOLS.POSTMORTEM_ADD, {
      title,
      notes,
  ...
  /**
   * Get a postmortem by id. Returns the textual representation provided by the backend.
   */
  async getPostmortem(id: string): Promise<string> {
    const resp = await this.callTool<MCPToolResponse>(
      MCP_TOOLS.POSTMORTEM_GET,
      { id }
    );
    const text =
      (resp as any)?.content?.[0]?.text ?? (resp as any)?.result ?? "";
    return typeof text === "string" ? text : JSON.stringify(text);
  }

  /**
   * Mark a postmortem as resolved by id.
   */
  async resolvePostmortem(id: string): Promise<void> {
    await this.callTool(MCP_TOOLS.POSTMORTEM_RESOLVE, { id });
  }

  /**
   * Convert a postmortem's lessons into learnings records.
   */
  async postmortemToLearnings(id: string): Promise<void> {
    await this.callTool(MCP_TOOLS.POSTMORTEM_TO_LEARNINGS, { id });
  }
  ...
  async getCallers(symbol: string): Promise<CallGraphResult> {
    return this.callTool<CallGraphResult>(MCP_TOOLS.EXPLORE_CALLERS, {
      symbol,
    });
  }
  ...
  async getCallees(symbol: string, file: string): Promise<CallGraphResult> {
    return this.callTool<CallGraphResult>(MCP_TOOLS.EXPLORE_CALLEES, {
      symbol,
      file,
    });
  }
  ...
  async logActivity(
    sessionId: string,
    kind: "file_read" | "file_edit" | "search" | "command",
    target: string,
    outcome?: "success" | "failure" | "unknown"
  ): Promise<void> {
    await this.callTool(MCP_TOOLS.SESSION_LOG, {
      sessionId,
      kind,
      target,
      outcome,
  ...
  async checkConflict(
    path: string,
    sessionId?: string
  ): Promise<{ conflict: boolean; agent?: string }> {
  ...
  async listSessions(options?: {
    ...
    const result = await this.callTool<SessionListResult>(
      MCP_TOOLS.SESSION_LIST,
      {
   ...
   */
  async getCorridorLearnings(options?: {
    query?: string;
    limit?: number;
  }): Promise<CorridorLearning[]> {
    const result = await this.callTool<{
      learnings: CorridorLearning[];
      count: number;
    }>(MCP_TOOLS.CORRIDOR_LEARNINGS, options || {});
  ...
  async storeConversation(
    ...
    const result = await this.callTool<{ id: string }>(
      MCP_TOOLS.CONVERSATION_STORE,
      {
        summary,
        messages,
  ...
  async getLinks(
    ...
    const result = await this.callTool<{ links: RecordLink[] }>(
      MCP_TOOLS.RECALL_LINKS,
      {
        recordId,
  ...
  private async callTool<T>(toolName: string, args: any): Promise<T> {
    ...
    try {
      ...
      this.channel.appendLine(
        `[MCP] Tool error (${toolName}): ${error.message}`
      );
      throw error;
    }
  }
