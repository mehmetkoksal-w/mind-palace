...
func (g *GlobalCorridor) GetLinks() ([]LinkedWorkspace, error) {
	...
	for rows.Next() {
		...
		if addedAt.Valid {
			l.AddedAt, _ = time.Parse(time.RFC3339, addedAt.String)
		}
		if lastAccessed.Valid {
			l.LastAccessed, _ = time.Parse(time.RFC3339, lastAccessed.String)
		}
...
func scanPersonalLearnings(rows *sql.Rows) ([]PersonalLearning, error) {
	...
	for rows.Next() {
		...
		l.LastUsed, _ = time.Parse(time.RFC3339, lastUsed)
