...
└───────────────────┼─────────────────────────┘
    ...
    │               │               │
┌───┴───┐     ┌────┴────┐    ┌────┴────┐
│Butler │     │ Memory  │    │Corridor │
│(index)│     │(sessions│    │(global) │
└───────┘     │learnings│    └─────────┘
              └─────────┘
```
...
The Angular dev server proxies API requests to the Go backend.

### Embedding in Binary

The dashboard is embedded in the `palace` binary:
...
The dashboard uses modern JavaScript features and may not work in older browsers.
