...
func TestPalaceLifecycle(t *testing.T) {
	...
	runPalace(t, binPath, workspace, "collect", "--root", workspace)
	...
	runPalace(t, binPath, workspace, "verify", "--root", workspace)

	if err := os.WriteFile(seedPath, []byte("hello world\nupdated\n"), 0o644); err != nil {
		t.Fatalf("modify seed file: %v", err)
	}

	output := runPalaceExpectFail(t, binPath, workspace, "verify", "--root", workspace)
...
func buildPalace(t *testing.T, repoRoot, binPath string) {
	t.Helper()
	cmd := exec.Command("go", "build", "-o", binPath, "./cmd/palace")
	cmd.Dir = repoRoot
